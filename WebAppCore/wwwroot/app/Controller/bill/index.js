var BillController = function () {
    ...
    function getProductProduct(listBillDeatail) {
        var response = [];
        $.each(listBillDeatail, function (i,data) {
            response.push(data.Product);
        });
        cachedObj.products = response;
    }
    ...
    function getBillStatusName(status) {
        var statusObj = $.grep(cachedObj.billStatuses, function (element, index) {
            return element.Value === status;
        });
        if (statusObj.length > 0)
            return statusObj[0].Name;
        else return '';
    }
